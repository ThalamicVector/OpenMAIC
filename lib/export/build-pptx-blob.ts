/**
 * Server-safe PPTX builder (no React / no "use client").
 * Used by EduMind export API and by the client useExportPPTX hook.
 */
...
import { svgMarkupToBase64 } from '@/lib/export/svg2base64';
...
export type PptxMediaLookup = (
  key: string,
) => { status?: string; objectUrl?: string; poster?: string } | undefined;
    ...
    for (const el of slide.elements) {
      ...
      // ── SHAPE ──
        ...
        if (el.special) {
          ...
          const strokeAttrs = el.outline?.color
            ? ` stroke="${escapeXmlAttr(el.outline.color)}" stroke-width="${el.outline.width || 1}"`
            : '';
          const svgMarkup =
            `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${el.viewBox[0]} ${el.viewBox[1]}" ` +
            `width="${el.width}" height="${el.height}">` +
            `<path d="${escapeXmlAttr(el.path)}" fill="${escapeXmlAttr(el.fill || 'none')}"${strokeAttrs}/>` +
            `</svg>`;
          const base64SVG = svgMarkupToBase64(svgMarkup);
      ...
      // ── LATEX ──
        ...
        if (omml) {
          ...
          const svgMarkup =
            ...
            `viewBox="${vbX} ${vbY} ${vbW} ${vbH}" stroke="${escapeXmlAttr(el.color || '#000000')}" ` +
            `stroke-width="${sw}" fill="none" stroke-linecap="round" stroke-linejoin="round">` +
            `<path d="${escapeXmlAttr(el.path)}"/>` +
            `</svg>`;
          const base64SVG = svgMarkupToBase64(svgMarkup);
      ...
      // ── VIDEO / AUDIO ──
        ...
        if (mediaLookupKey) {
          const task = mediaLookup?.(mediaLookupKey);
          if (task?.status === 'done' && task.objectUrl) {
            resolvedSrc = task.objectUrl;
        ...
        try {
          ...
          if (el.type === 'video') {
            ...
            // 2. Fallback: capture first frame from video via canvas (browser only)
            if (!coverBase64 && typeof document !== 'undefined') {
              try {
